...
import { CepAutocomplete, CepData } from "./CepAutocomplete";
import { 
...
export const SmartAddressInput = ({
  ...
  useEffect(() => {
    if (!isMapLoaded || !mapVisible || !mapRef.current || mapInstanceRef.current) return;
  ...
  const handleCepFound = (cepData: CepData) => {
    ...
    if (geocoderRef.current && cepData.address) {
      const fullAddress = `${cepData.address}, ${cepData.neighborhood}, ${cepData.city} - ${cepData.state}, Brasil`;
        ...
        <div className="space-y-3 pt-2">
          ...
          <div className={compact ? "grid grid-cols-4 gap-2" : "grid grid-cols-3 gap-3"}>
            <div className={compact ? "col-span-3" : "col-span-2"}>
              ...
              <div className="relative">
                <Home className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-muted-foreground" />
                <Input
                  id="address"
                  value={value.address}
